...
-   상속과 훅을 이용해 사용자 맞춤형으로 만들 수도 있다.
    ...
    -   객체는 지원하지만 일급 함수 first-class function을 지원하지 않는 프로그래밍 언어를 사용할 때는 명령을 이요해 일급 함수의 기능 대부분을 흉내 낼 수 있다.
...
-   '명령' : 이 맥락에서의 명령은 요청을 캡슐화한 객체로, 다지인 패턴 중 명령 패턴 command pattern 에서 말하는 명령과 같다.
