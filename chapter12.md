...
-   슈퍼클래스 추출하기의 대안으로는 클래스 추출하기(7.5절)가 있다. - 어느 것을 선택하느냐는 중복 동작을 상속으로 해결하느냐 위임으로 해결하느냐에 달렸다.
-   슈퍼클래스 추출하기 방법이 더 간단할 경우가 많으니 이 리팩터링을 먼저 시도해보시길 권한다. - 나중에라도 필요해지면 슈퍼클래스를 위임으로 바꾸기(12.11절)는 어렵지 않다.
